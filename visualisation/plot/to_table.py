...
class ToTable:
    ...
    def __init__(
        ...
        self.save_file_name_average_ranking_df_archi = (
            "average_ranking_archi.txt"
        )
        self.save_file_name_average_ranking_df_pooling = (
            "average_ranking_pooling.txt"
        )
    ...
    def set_transpose(self) -> None:
        ...
        for dic in self.list_dict:
            ...
            dic_results["global_pooling_layer"].append(
                dic["global_pooling_layer"]
            )
            dic_results["local_pooling_layer"].append(
                dic["local_pooling_layer"]
            )
    ...
    def filter(self) -> None:
        self.df = self.df[
            self.df["global_pooling_layer"] == self.global_pooling
        ]

    def to_bold(self) -> None:
        """
        Bold the values of the dataframe (self.df)
        that corresponds to the maximum
        """
        indexes_to_bold = self.df.groupby("dataset")["mean_accuracy"].idxmax()

        self.df["mean_accuracy"] = (
            self.df["mean_accuracy"].apply("{:.3f}".format).astype(str)
        )
        self.df["std_accuracy"] = (
            self.df["std_accuracy"].apply("{:.3f}".format).astype(str)
        )

        self.df["accuracy"] = self.df.apply(
            lambda row: "${} \\pm {}$".format(
                row["mean_accuracy"], row["std_accuracy"]
            ),
            axis=1,
        )

        self.df.loc[indexes_to_bold, "accuracy"] = self.df.loc[
            indexes_to_bold
        ].apply(
            lambda row: "$\\bm{{{} \\pm {}}}$".format(
                row["mean_accuracy"], row["std_accuracy"]
            ),
            axis=1,
        )
    ...
    def rename_columns(self) -> None:
        """
        Rename the columns to reduce
        the size of the column of the future latex table
        """
    ...
    def pivot(self) -> None:
        """
        Use the function 'pivot' of pandas
        to 'group by' datasets (multicolumns)
        """
        if self.global_pooling == "":
    ...
    def set_ranking(self) -> None:
        """
        Set a new attribute to be the argsort
        of the mean accuracy (after the groupby)
        """
    ...
    def set_conv_and_pool_map(self) -> None:
        ...
        self.pool_map = self.df.reset_index(level=self.level)[
            "Local"
        ].to_dict()
    ...
    def set_best(self) -> None:
        ...
        self.df_best_pooling = (
            self.aggregator(self.df_ranking)
            .reset_index(level=self.level)
            .groupby(["Conv"])
            .idxmin()[self.aggregated_columns]
            .copy()
        )
        ...
        self.best_indexes_by_architecture = list(
            set(self.df_best_pooling.values.flatten())
        )
        self.best_indexes_by_pooling = list(
            set(self.df_best_architecture.values.flatten())
        )

        self.df_best_pooling = self.df_best_pooling.map(
            lambda x: self.pool_map[x]
        )
        self.df_best_architecture = self.df_best_architecture.map(
            lambda x: self.conv_map[x]
        )
    ...
    def set_worst(self) -> None:
        ...
        self.df_worst_pooling = (
            ...
            .reset_index(level=self.level)
            .groupby(["Conv"])
            .idxmax()[self.aggregated_columns]
            .copy()
        )
        ...
        self.df_worst_pooling = self.df_worst_pooling.map(
            lambda x: self.pool_map[x]
        )
        self.df_worst_architecture = self.df_worst_architecture.map(
            lambda x: self.conv_map[x]
        )
    ...
    def modify_training_time(self) -> None:
        ...
        """
        training_time = (
            self.df["training_time"].sum(axis=1).copy().astype(int).astype(str)
        )
    ...
    def set_average_ranking(self) -> None:
        ...
        self.average_ranking_df_archi = (
            self.df_ranking.reset_index(level=[0])
            .groupby(["Conv"])[self.datasets]
            .min()
            .astype(int)
        )
        self.average_ranking_df_pooling = (
            self.df_ranking.reset_index(level=[1])
            .groupby(["Local"])[self.datasets]
            .min()
            .astype(int)
        )
    ...
    def set_df_by(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        respectively
        """
        self.df_by_architecture = self.df.iloc[
            self.best_indexes_by_architecture
        ].copy()
        self.df_by_pooling = (
            self.df.iloc[self.best_indexes_by_pooling]
            .copy()
            .reorder_levels(self.level_transpose)
            .sort_index()
        )
    ...
    def save_all(self) -> None:
        ...
        ToTable.save_df_latex(
            self.df_by_architecture,
            self.save_path,
            self.save_file_name_df_by_architecture,
        )
        ToTable.save_df_latex(
            self.df_by_pooling,
            self.save_path,
            self.save_file_name_df_by_pooling,
        )
        ToTable.save_df_latex(
            self.df_best_architecture,
            self.save_path,
            self.save_file_name_df_best_architecture,
        )
        ToTable.save_df_latex(
            self.df_worst_architecture,
            self.save_path,
            self.save_file_name_df_worst_architecture,
        )
        ToTable.save_df_latex(
            self.df_best_pooling,
            self.save_path,
            self.save_file_name_df_best_pooling,
        )
        ToTable.save_df_latex(
            self.df_worst_pooling,
            self.save_path,
            self.save_file_name_df_worst_pooling,
        )
