"""A simple API to expose our trained model over molecules classification"""
...
app = FastAPI(
    ...
    description="This is done over 3 possible datasets." +
    "Please enter MUTAG, ENZYMES, PROTEINS in mol_type.",
)
...
async def bestmodel(mol_type: str) -> dict:
...
async def predict(file: UploadFile, mol_type: str) -> int:
    ...
    conv_method = conv_selection(
        best_model_config["convolution_layer"],
        best_model_config["attention_heads"],
    )
    global_pool_method = global_pooling_selection(
        best_model_config["global_pooling_layer"]
    )
    local_pool_method, dic_conversion_layer = local_pooling_selection(
        best_model_config["local_pooling_layer"], device="cpu"
    )
    ...
    model.load_state_dict(
        torch.load(
            f"model/weights/{mol_type}"
            f"_{best_model_config['convolution_layer']}"
            f"_{best_model_config['global_pooling_layer']}"
            f"_{best_model_config['local_pooling_layer']}"
        )
    )
    model.eval()
