...
if not("AWS_S3_ENDPOINT" in os.environ) or not("AWS_ACCESS_KEY_ID" in os.environ) or not("AWS_SECRET_ACCESS_KEY" in os.environ) or not("AWS_SESSION_TOKEN" in os.environ):
    raise PermissionError("No credentials in .env detected. Check if the .env file exist and is properly filled.")
...
def download_S3_folder(bucket, S3_directory, local_directory):
    ...
    for dataset_path in fs.ls(f'{bucket}/{S3_directory}'):
        ...
        for file in tqdm(fs.ls(f'{directory_path}/{folder_name}'), desc=f'Downloading - {dataset_name} '):
            _, filename = os.path.split(file)
            if not (filename in os.listdir(outpath)):
                with fs.open(file) as f:
                    fs.download(f, f'{local_directory}/{dataset_name}/{folder_name}')
